...
def norm_standard(CTG_features, selected_feat=('LB', 'ASTV'), mode='none', flag=False):
    ...
    x_discreption = CTG_features[x].describe()
    y_discreption = CTG_features[y].describe()

    first = CTG_features[x]
    second = CTG_features[y]
    nsd_res = pd.DataFrame()

    if mode == 'standard':
        first = (first - x_discreption['mean'])/(x_discreption['std'])
        second = (second - y_discreption['mean']) /(y_discreption['std'])
        data = {'col1': first, 'col2':second}
        nsd_res = pd.DataFrame(data=data)


    if mode == 'MinMax':
        first = (first - x_discreption['min'])/(x_discreption['max'] - x_discreption['min'])
        second = (second - y_discreption['min']) / (y_discreption['max'] - y_discreption['min'])
        data = {'col1': first, 'col2': second}
        nsd_res = pd.DataFrame(data=data)


    if mode == 'mean':
        first = (first - x_discreption['mean'])/(x_discreption['max'] - x_discreption['min'])
        second = (second - y_discreption['mean']) / (y_discreption['max'] - y_discreption['min'])
        data = {'col1': first, 'col2':second}
        nsd_res = pd.DataFrame(data=data)
